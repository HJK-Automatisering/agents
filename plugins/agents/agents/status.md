...
2. Frontmatter i hver fil under `docs/specs/`, `docs/plans/`, `docs/tests/`, `docs/findings/`, `docs/rca/`: `nummer`, `titel`, `status`.
...
5. Projektets `CLAUDE.md` hvis den findes — så du kender stakken når du foreslår næste skridt.
...
VENTER PÅ DIG
- 0008 spec er `udkast` og skal godkendes før architect kan gå videre
  → docs/specs/0008-...md
...
5. **Fund er udført, alt er grønt** → næste nummer fra `## Kommende` på `BOARD.md`, rolle `brainstorm`.
6. **Intet af ovenstående, og der er ingen specs eller commits** → det er projekt nul, rolle `kickoff`.
